...
void SCManager::buildRingKeyKDTree(KeyMat &polarcontext_invkeys_mat, std::vector<Eigen::MatrixXd>& polarcontexts) {
    ...
    polarcontext_invkeys_to_search_.assign( polarcontext_invkeys_mat.begin(), polarcontext_invkeys_mat.end() ) ;

    polarcontext_tree_.reset();
    polarcontext_tree_ = std::make_unique<InvKeyTree>(PC_NUM_RING /* dim */, polarcontext_invkeys_to_search_, 10 /* max leaf */ );
}

std::pair<int, float> SCManager::detectClosestMatch(Eigen::MatrixXd &sc, std::vector<float> &ringkey, Eigen::MatrixXd &sectorkey, double &score) {
    int loop_id { -1 };
    if (polarcontexts_.size() <= 0) {
   ...
   /* for ( int i = 240; i < 260; i++ )
    {
        MatrixXd polarcontext_candidate = polarcontexts_[i];
        std::pair<double, int> sc_dist_result = distanceBtnScanContext( sc, polarcontext_candidate );
...
std::pair<int, float> SCManager::detectLoopClosureID ( void )
{
    ...
    if( tree_making_period_conter % TREE_MAKING_PERIOD_ == 0 ) // to save computation cost  // TODO not right ( tree_making_period_conter % TREE_MAKING_PERIOD_ == 0)
    {
        polarcontext_invkeys_to_search_.clear();

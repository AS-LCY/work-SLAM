...
//                 pl_full[i].z = msg->points[i].z;
//                 pl_full[i].intensity = msg->points[i].reflectivity;
//                 pl_full[i].curvature = msg->points[i].offset_time * float(1000); // use curvature as time of each
//                 laser points, curvature unit: ms
	...
	for (uint i = 1; i < plsize; i++) { // zd delete (msg->points[i].line < N_SCANS)
		if (((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00) &&
			...
			if (valid_num % point_filter_num == 0) {
				...
				if ((abs(pl_full[i].x - pl_full[i - 1].x) > 0.15) || (abs(pl_full[i].y - pl_full[i - 1].y) > 0.15) ||
					(abs(pl_full[i].z - pl_full[i - 1].z) > 0.15)) {
					if (range > (blind_square)) {
						pl_surf.push_back(pl_full[i]);
					}
				} // if

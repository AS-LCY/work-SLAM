...
void BackEnd::addOdomFactor(Eigen::Isometry3d transformTobeMapped) {
	if (KeyPoint_->points.empty()) { // first keyframe node
		...
		// TODO(jxl): 第一个关键帧的位置是0吗，最开始要静止状态
	} else {
...
bool BackEnd::saveKeyFramesAndFactor(Eigen::Isometry3d transformTobeMapped, PointCloudType::Ptr lidar_cloud,
	...
	thisPose3D.intensity = KeyPoint_->size(); // TODO(jxl): 应该用KeyPoses_来判断

	std::unique_lock<std::mutex> keyframes_lock(mtxPose_);
	KeyPoint_->push_back(thisPose3D);
...
void BackEnd::performLoopClosure(double time) {
	...
	*CopyKeyPoint_ = *KeyPoint_;
	...
	{
		// 提取当前关键帧特征点集合，降采样
		loopFindNearKeyframes(cureKeyframeCloud, loopKeyCur, 0); //  将cur keyframe转换到world系下
		// 提取闭环匹配关键帧前后相邻若干帧的关键帧特征点集合，降采样
		loopFindNearKeyframes(prevKeyframeCloud, loopKeyPre, 20); // 选取20个keyframe拼成submap
	}
	...
	gtsam::Pose3 poseTo = gtsam::Pose3(CopyKeyPoses_[loopKeyPre].pose.matrix().cast<double>()); // 闭环匹配帧的位姿
...
// 		return;
// 	} else {
// 		std::cout << "ICP fitness test passed (" << icp.getFitnessScore() << " < " << historyKeyframeFitnessScore
// 				  << "). Add this SC loop." << std::endl;
// 	}
// 	std::cout << "SC loop found! between " << loopKeyCur << " and " << loopKeyPre << "." << std::endl; // giseop
...
// 	fixed frame pcl::getTranslationAndEulerAngles(tCorrect, x, y, z, roll, pitch, yaw);

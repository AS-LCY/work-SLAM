...
ImuProcess::ImuProcess() : b_first_frame_(true), imu_need_init_(true), start_timestamp_(-1) {
	...
	mean_gyr		= V3D(0, 0, 0);
	angvel_last		= Vector3d(0, 0, 0);	//上一帧角速度初始化
...
void ImuProcess::IMU_init(const MeasureGroup& meas, esekfom::esekf& kf_state, int& N) {
	...
	if (b_first_frame_) //如果为第一帧IMU
	{
		...
		mean_acc << imu_acc[0], imu_acc[1], imu_acc[2];				 //第一帧加速度值作为初始化均值
		mean_gyr << gyr_acc[0], gyr_acc[1], gyr_acc[2];				 //第一帧角速度值作为初始化均值
	...
	// std::cout << "IMU init new -- init_state  " << init_state.pos  <<" " << init_state.bg <<" " << init_state.ba <<"
	// " << init_state.grav << std::endl;
}
...
void ImuProcess::UndistortPcl(const MeasureGroup& meas, esekfom::esekf& kf_state, PointCloudType& pcl_out) {
	...
	sort(pcl_out.points.begin(), pcl_out.points.end(),
		 time_list); // 放在sync_packages 中进行排序，这里curvature中存放了时间戳（在preprocess.cpp中）
	...
	for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++) {
		...
		if (tail->time_stamp < last_lidar_end_time_) continue;
		...
		acc_s_last = V3D(tail->linear_acceleration[0], tail->linear_acceleration[1], tail->linear_acceleration[2]) *
		...
		IMUpose.push_back(
			set_pose6d(offs_t, acc_s_last, angvel_last, imu_state.vel, imu_state.pos, imu_state.rot.matrix()));
	}
	...
	for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--) {
		...
		//此时从IMUpose的末尾开始循环，也就是从时间最大处开始，因此只需要判断 点云时间需>IMU head时刻  即可   不需要判断
		//点云时间<IMU tail
...
void ImuProcess::Process(const MeasureGroup& meas, esekfom::esekf& kf_state, PointCloudType::Ptr& cur_pcl_un_) {
	...
	if (imu_need_init_) {
		...
		if (init_iter_num > MAX_INI_COUNT) {
			cov_acc *= pow(G_m_s2 / mean_acc.norm(), 2); // zx ？？

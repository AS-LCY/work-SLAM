...
CloudMap::CloudMap(){
    load_sc_info_.clear();
    loaded_cloud_map_.reset(new PointCloudXYZI());
...
bool CloudMap::load_cloud_map(std::string map_dir){
    ...
    if (std::filesystem::exists(cloud_map_file_path)){
        pcl::io::loadPCDFile(cloud_map_file_path, *loaded_cloud_map_);
        std::cout <<"map loaded --- points count: "<<loaded_cloud_map_->points.size() << std::endl;
    }else {
    ...
    key_point_.reset(new pcl::PointCloud<pcl::PointXYZ>());
    ...
    while (std::getline(file, line)) {        
        ...
        key_point_->push_back(point);// 数据保存 ---------------------------------------------------------
        ...
        polarcontext_invkeys_mat_.push_back(eig2stdvec(ringkey));  // 数据转换后保存 -----------------------
        polarcontexts_.push_back(sc); // 数据保存 ---------------------------------------------------------
        //   if (i == 17)
        //    std::cout << " \n matrix:"  << std::endl;
        //    std::cout << value <<",";
        load_sc_info_.push_back(read_one_line);// 数据保存 ------------------------------------------------
    }
    file.close();

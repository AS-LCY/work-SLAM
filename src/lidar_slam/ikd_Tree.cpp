...
void KD_TREE<PointType>::lasermap_fov_segment(Vector3d pos_LiD) {
	...
	if (cub_needrm.size() > 0) kdtree_delete_counter = Delete_Point_Boxes(cub_needrm);
	kdtree_delete_time = omp_get_wtime() - delete_begin;
}

template <typename PointType>
void KD_TREE<PointType>::map_incremental(PointTypeCloud feats_down_world, const vector<PointVector>& Nearest_Points,
										 bool flg_EKF_inited) {
	...
	for (int i = 0; i < feats_down_size; i++) {
		...
		if (!Nearest_Points[i].empty() &&
			flg_EKF_inited) // Nearest_Points是当前帧看到的点云的附近点，Nearest_Points[i]就是第i个点云点的附近点（之前搜索有距离限制）
		{
			...
			PointType downsample_result, mid_point;
			mid_point.x =
				floor(feats_down_world->points[i].x / downsample_size) * downsample_size + 0.5 * downsample_size;
			mid_point.y =
				floor(feats_down_world->points[i].y / downsample_size) * downsample_size + 0.5 * downsample_size;
			mid_point.z =
				floor(feats_down_world->points[i].z / downsample_size) * downsample_size + 0.5 * downsample_size;
			float dist = calc_dist(feats_down_world->points[i], mid_point); //当前点与box中心的距离

			//判断最近点在x、y、z三个方向上，与中心的距离，判断是否加入时需要降采样
			if (fabs(points_near[0].x - mid_point.x) > 0.5 * downsample_size &&
				fabs(points_near[0].y - mid_point.y) > 0.5 * downsample_size &&
				fabs(points_near[0].z - mid_point.z) > 0.5 * downsample_size) {

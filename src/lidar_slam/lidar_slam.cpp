...
void LidarSlam::reset(SlamWorkMode work_mode, rclcpp::Node::SharedPtr node) {
	...
	// back_end_.reset(new BackEnd(key_frame_distance, key_frame_angle, loopSearchDistance));
	...
	// p_lidar_pre_.reset(new Preprocess());
	// p_lidar_pre_->set(config_param_.lidar_preproc);
	...
	// show_thread_.reset(new std::thread(&LidarSlam::showThread, this));
...
bool LidarSlam::sync_packages(MeasureGroup& meas) {
	if (lidar_buffer_.empty() || imu_buffer_.empty()) {
		...
		// ROS_INFO_STREAM("buffer empty, sync_packages failed !, lidar size: " << lidar_buffer_.size() << ", imu size:
		// "
		// << imu_buffer_.size());
		return false;
	}
	if (reseting_ == true) {
		cout << "reseting_, sync_packages return !" << endl;
		// ROS_INFO_STREAM("reseting_, sync_packages return !");
		return false;
	}
	if (lidar_pushed_ && omp_get_wtime() - time_buffer_.front() > 1.5 * 0.1) {
		cout << "lidar lose rate: " << omp_get_wtime() - time_buffer_.front() << endl;
		// ROS_WARN_STREAM(RED << "lidar lose rate: " << omp_get_wtime()-time_buffer_.front() << RESET);
	}
	/*** push a lidar scan ***/
	...
	if (!lidar_pushed_) {
		...
		// {
		//     lidar_end_time_ = meas.lidar_beg_time + lidar_mean_scantime_; // 记录lidar结束时间为 起始时间 +
		//     单帧扫描时间 ROS_WARN_STREAM(YELLOW << "Too few input point cloud!" << RESET);
		// }
		// else if (meas.lidar->points.back().curvature / double(1000) < 0.5 * lidar_mean_scantime_) //最后一个点的时间
		// 小于 单帧扫描时间的一半
		// {
		//     lidar_end_time_ = meas.lidar_beg_time + lidar_mean_scantime_; // 记录lidar结束时间为 起始时间 +
		//     单帧扫描时间
		...
		//     // 动态更新每帧lidar数据平均扫描时间
		//     lidar_mean_scantime_ += (meas.lidar->points.back().curvature / double(1000) - lidar_mean_scantime_) /
		//     scan_num_;
		// }
		...
		lidar_mean_scantime_ = meas.lidar->points.back().curvature * 0.001;
		lidar_mean_scantime_ = lidar_mean_scantime_ < 0.1 ? 0.1 : lidar_mean_scantime_;
		lidar_mean_scantime_ = lidar_mean_scantime_ > 0.15 ? 0.1 : lidar_mean_scantime_;
		lidar_end_time_ = meas.lidar_beg_time + lidar_mean_scantime_;

		if (lidar_mean_scantime_ > 0.11) {
			cout << "lidar scan time: " << lidar_mean_scantime_ << endl;
			// ROS_INFO_STREAM(RED << "lidar scan time: " << lidar_mean_scantime_ << RESET);
		}
	...
	if (last_timestamp_imu_ < lidar_end_time_) {
		...
		// ROS_WARN_STREAM(YELLOW << "last_timestamp_imu_: " << setprecision(15) << last_timestamp_imu_ << RESET);
		// ROS_WARN_STREAM(YELLOW << "lidar_end_time_: " << setprecision(15)  << lidar_end_time_ << RESET);
		return false;
	}
	...
	// ROS_INFO_STREAM(BOLDYELLOW << "Measures_.imu.size(): "<< meas.imu.size() << RESET);
...
void LidarSlam::localizationThread() {
	...
	while (thread_run_ && reseting_ == false) {
		...
		// UndistortCloudInOdom_test.reset(new PointCloudType());
		{
			...
			// downSizeFilterCloud_test_.setInputCloud(UndistortCloudInOdom_);
			// downSizeFilterCloud_test_.filter(*UndistortCloudInOdom_test);
		...
		if (local_thrd_status_.load() == 2) { // 重定位失败
			...
			if (!globalLocalizationSuccess_) {
				...
				if (!getLoadMap()) {
					...
					// localization_->globalLocalization(undistortCloud_,T_odom_lidar_,p_imu_->initial_rotate,score_thr);
					globalLocalizationSuccess_ = localization_->globalLocalization(
						FilteredUndistortCloud_test, T_odom_lidar_, p_imu_->initial_rotate, score_thr);
			...
			if (!globalLocalizationSuccess_) {
				...
				if (!UndistortCloudInOdom_ || UndistortCloudInOdom_->points.size() == 0) {
					...
					globalLocalizationSuccess_ = global_localization_->global_localize(
						undistortCloud_, T_odom_lidar_, p_imu_->initial_rotate, score_thr);
					// globalLocalizationSuccess_ =
					// localization_->globalLocalization(undistortCloud_,T_odom_lidar_,p_imu_->initial_rotate,
					// score_thr);
...
void LidarSlam::lidar_pcl_cbk(const PointCloudType::Ptr cloud) {
	...
	if (time_sync_en_ && !timediff_set_flg_ && abs(curr_time - last_timestamp_imu_) > 1 && !imu_buffer_.empty()) {
	...
	lidar_buffer_.push_back(cloud); //储存处理后的lidar特征
...
curr_time);
//         ROS_WARN_STREAM(YELLOW << "IMU and LiDAR not Synced, IMU time: "<< last_timestamp_imu_ << ", lidar header
...
//     if (msg->time_stamp < last_timestamp_lidar_)
//     {
//         // printf("lidar loop back, clear buffer");
//         ROS_WARN_STREAM(YELLOW << "lidar loop back, clear buffer"<< RESET);
//         lidar_buffer_.clear();
//         // cout<<"************************* lidar_buffer_ clear *********"<<endl;
//         ROS_WARN_STREAM(YELLOW<<"************************* lidar_buffer_ clear *********"<<RESET);
//     }
//     //   else if (msg->time_stamp - last_timestamp_lidar_ > 1.5 * 0.05){
//     //     // printf("lidar lose rate");
//     //     ROS_ERROR_STREAM(RED << "lidar lose rate" << RESET);
//     // }
//     last_timestamp_lidar_ = msg->time_stamp;
...
//         ROS_WARN_STREAM(YELLOW << "IMU and LiDAR not Synced, IMU time: "<<last_timestamp_imu_ <<", lidar header time:
"<< last_timestamp_lidar_<<RESET);
//     }
...
//         // printf("Self sync IMU and LiDAR, time diff is %.10lf \n", timediff_lidar_wrt_imu_);
//         ROS_INFO("Self sync IMU and LiDAR, time diff is %.10lf ", timediff_lidar_wrt_imu_);
//     }

//     PointCloudType::Ptr ptr(new PointCloudType());

//     // 特征提取或间隔采样
//     // p_lidar_pre_->process(msg, ptr);
//     lidar_pre_ptr_->pre_process(msg, ptr);

//     // {
//     //     std::lock_guard<std::mutex> lk(mtx_obstacle_cloud_);
//     //     ObstacleCloud_->points.clear();
//     //     FilteredObstacleCloud_->points.clear();
//     //     // ObstacleCloud_ = transformPointCloud(p_lidar_pre_->pl_obstacle, param.T_wheel_lidar);
//     //     int size = p_lidar_pre_->pl_obstacle->points.size();
//     // }
...
//         time_buffer_.clear();
//     }
//     lidar_buffer_.push_back(ptr); //储存处理后的lidar特征
//     // cout<<"********************* lidar_buffer_ push back *********"<<endl;
...
void LidarSlam::imu_cbk(const std::shared_ptr<livox_ros::ImuMsg>& msg_in) { // base_link下的acc，gyro
	...
	std::lock_guard<std::mutex> lk(mtx_buffer_);
	// double delta_time = curr_timestamp_imu - last_timestamp_imu_;
	// if(delta_time < 0 && delta_time > jump_back_time_thr){
	//     ROS_ERROR_STREAM("imu cbk: time jump back, curr_time - last_time = " << delta_time<<endl;
	//     // last_timestamp_imu_ 不更新，等待最新时间 到达 跳变前的时间
	//     return;
	// }
	if (curr_timestamp_imu < last_timestamp_imu_) {
		cout << YELLOW << "imu loop back, clear buffer" << RESET << endl;
		imu_buffer_.clear();
	} else if (curr_timestamp_imu - last_timestamp_imu_ > 1.5 * 0.1) {
		cout << YELLOW << "imu lose rate" << RESET << endl;
	}
	imu_buffer_.push_back(msg);
	// cout<<"************************* imu_buffer_ push back *********"<<endl;
	last_timestamp_imu_ = curr_timestamp_imu; // update imu time
	localization_wait_ = true;
	// if (globalLocalizationSuccess_||!param.localization_mode){// TODO add lock

	// std::lock_guard<std::mutex> lk2(mtx_pose_);
	if (globalLocalizationSuccess_ || working_mode_ == MAPPING || working_mode_ == SEC_MAPPING) { // TODO add lock
		if (current_pose_.base_time < localization_base_.base_time - 0.005) {
			// std::cout << "predicate pose "<<current_pose_.imu_state.pos.transpose()<<std::endl;
			// std::cout << "update pose "<<localization_base_.imu_state.pos.transpose()<<std::endl;
			...
			for (auto it = imu_buffer_.begin(); it != imu_buffer_.end(); it++) {
				...
				if (msg->time_stamp > localization_base_.update_time) {
					...
					// current_pose_.imu_state.rot = current_pose_.imu_state.rot * Sophus::SO3::exp(angvel * dt);
	...
	// poses_buffer_.push_back(std::pair(curr_timestamp_imu,getLidarInOdom())); // 此为c++17用法
	poses_buffer_.push_back(std::make_pair(curr_timestamp_imu, getLidarInOdom())); // 此为c++11用法
	if (poses_buffer_.size() > 200) poses_buffer_.pop_front();
...
void LidarSlam::delete_log_file(double keep_time) { // about 100MB pr 60s
	if (pcd_file_.size() < 10) return;
	if (pcd_file_.back() - pcd_file_.front() > keep_time) {
		// std::filesystem::remove(config_param_.common.save_log_dir + std::to_string(pcd_file_.front()) + ".pcd");
...
bool LidarSlam::run() {
	...
	// cout<<"lidar buffer size: "<<lidar_buffer_.size()<<endl;
	// cout<<"imu   buffer size: "<<imu_buffer_.size()<<endl;
	...
	if (sync_packages(Measures_)) {
		...
		if (flg_first_scan_) {
			...
			p_imu_->first_lidar_time = first_lidar_time_; //记录第一帧绝对时间
			flg_first_scan_ = false;
			cout << "***************** flg_first_scan_ ********" << endl;
			return false;
		}
		...
		// 根据imu数据序列和lidar数据，向前传播纠正点云的畸变, 此前已经完成间隔采样或特征提取
		{
			...
			p_imu_->Process(Measures_, kf_, undistortCloud_);
		...
		// 不优化外参数就提出去 Eigen::Isometry3d T_odom_b(Sophus::SE3(state_point.rot, state_point.pos).matrix());
		{
			...
			T_odom_lidar_ = T_odom_b * T_b_lidar; // TODO check this
		}
		//  pos_lid = state_point.pos + state_point.rot * state_point.offset_T_L_I; // global系 lidar位置
		t1 = omp_get_wtime();
		if (undistortCloud_->empty() || (undistortCloud_ == NULL)) {
		...
		ikdtree_->lasermap_fov_segment(
			T_odom_lidar_.translation()); // 根据lidar在W系下的位置，重新确定局部地图的包围盒角点，移除远端的点
		t2 = omp_get_wtime();
		/*** downsample the feature points in a scan ***/
		// ROS_INFO("undistortCloudcount: %d", undistortCloud_->points.size());
		downSizeFilterCloud_.setInputCloud(undistortCloud_); // zx 0.5?
		...
		if (ikdtree_->Root_Node == nullptr) {
			...
			if (feats_down_size > feats_down_size_thr_) {
				ikdtree_->set_downsample_param(config_param_.ikdtree.map_leaf_size); // 0.5 默认0.2
				...
				ikdtree_->Build(FilteredUndistortCloudInOdom->points);
			}
			// std::cout << "build ikdtree_! "<<feats_down_size<< std::endl;
			cout << GREEN << "build ikdtree_! " << feats_down_size << RESET << endl;
			return false;
		}
		...
		kf_.update_iterated_dyn_share_modified(0.001, FilteredUndistortCloud_, *ikdtree_, Nearest_Points, 4, false);
		...
		T_odom_lidar_ = T_odom_b * T_b_lidar; // TODO check this
		//  double t_update_end = omp_get_wtime();
		//   while(localization_wait_){ // TODO check
		...
		if (working_mode_ == MAPPING || working_mode_ == SEC_MAPPING) {
			if (working_mode_ == SEC_MAPPING && !globalLocalizationSuccess_) {
				...
				loop_closure_wait_ = true;

				// cout<<"************* backend: checking keyPosesCount: "<<back_end_->getKeyframePoses().size()<<endl;
				...
				if (insert) {
					...
					// cout<<"debug: loaded_key_clouds_ready_: "<<back_end_->get_loaded_key_cloud_status()<<endl;
		...
		// std::cout<<"test "<< R2ypr(T_odom_lidar_.matrix().block<3, 3>(0, 0)).transpose() << std::endl;
		// if (!param.localization_mode){
		// if (working_mode_==MAPPING || working_mode_ == SEC_MAPPING){
		// }
		...
		t4 = omp_get_wtime();
		ikdtree_->map_incremental(FilteredUndistortCloudInOdom, Nearest_Points, flg_EKF_inited);
		t5 = omp_get_wtime();
		{
			frame_num++;
		...
		if (run_end - run_start > config_param_.common.slam_lose_rate_time_thr) {
			...
			// ms;"); ROS_INFO_STREAM("main: transform undistortCloud_  , time cost: " <<
			// (t1_transform-t0_transform)*1000 << " ms;"); ROS_INFO_STREAM("transform FilteredUndistortCloud_, time
			// cost: " << (t4-t3)*1000 << " ms;"); ROS_INFO_STREAM("ikdtree_->map_incremental        , time cost: " <<

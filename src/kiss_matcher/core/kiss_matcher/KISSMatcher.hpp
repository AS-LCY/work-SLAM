...
struct KISSMatcherConfig {
	...
	KISSMatcherConfig(
		...
		if (robin_noise_bound_gain < solver_noise_bound_gain) {
			throw std::runtime_error("`solver_noise_bound_gain` (" + std::to_string(solver_noise_bound_gain) +
...
class KISSMatcher {
   ...
   public:
	 ...
	 */
	RegistrationSolution estimate(const std::vector<Eigen::Vector3f>& src, const std::vector<Eigen::Vector3f>& tgt);

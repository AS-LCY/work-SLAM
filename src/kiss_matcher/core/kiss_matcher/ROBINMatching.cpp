...
ROBINMatching::ROBINMatching(const float noise_bound, const int num_max_corr, const float tuple_scale) {
...
std::vector<std::pair<int, int>> ROBINMatching::establishCorrespondences(
	std::vector<Eigen::Vector3f>& source_points, std::vector<Eigen::Vector3f>& target_points, Feature& source_features,
	Feature& target_features, std::string robin_mode, float tuple_scale, bool use_ratio_test) {
	pointcloud_.clear();
	features_.clear();
...
void ROBINMatching::match(const std::string& robin_mode, float tuple_scale, bool use_ratio_test) {
		...
		for (size_t j = r.begin(); j < r.end(); ++j) {
			...
			if (corres_K[j][0] >= 0 && corres_K[j][0] < nPti_) {
				if (i_to_j_multi_flann[corres_K[j][0]] == -1) {
					searchKDTree(feature_tree_j, features_[fi_][corres_K[j][0]], corres_K2[corres_K[j][0]],
								 dis_i[corres_K[j][0]], 1);
	...
	for (const auto& corres : matched_pairs) {
		corres_cross_checked_.emplace_back(std::pair<int, int>(std::get<0>(corres), std::get<1>(corres)));
	}
	...
	rejection_time_ =
		std::chrono::duration_cast<std::chrono::duration<double>>(t_rejection_end - t_rejection_init).count();
}
...
void ROBINMatching::runTupleTest(const std::vector<std::pair<int, int>>& corres,
								 std::vector<std::pair<int, int>>& corres_out, const float /*tuple_scale*/) {
	if (!corres.empty()) {
...
void ROBINMatching::applyOutlierPruning(const std::vector<std::pair<int, int>>& corres,
										std::vector<std::pair<int, int>>& corres_out, const std::string& robin_mode) {
	if (!corres.empty()) {
		size_t ncorr = corres.size();
		std::vector<bool> is_already_included(ncorr, false);
		corres_out.clear();
...
template <typename T>
void ROBINMatching::searchKDTree(const KDTree& tree, const T& input, std::array<int, 2>& indices,
								 std::array<float, 2>& dists, int nn) {
	size_t rows_t = 1;
...
template <typename T>
void ROBINMatching::searchKDTreeAll(KDTree* tree, const std::vector<T>& inputs, std::vector<int>& indices,

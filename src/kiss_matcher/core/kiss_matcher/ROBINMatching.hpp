...
class ROBINMatching {
   public:
	...
	ROBINMatching(const float noise_bound, const int num_max_corr = 5000, const float tuple_scale = 0.95);
	...
	std::vector<std::pair<int, int>> establishCorrespondences(
		...
		std::vector<Eigen::Vector3f>& source_points, std::vector<Eigen::Vector3f>& target_points,
		Feature& source_features, Feature& target_features, std::string robin_mode, float tuple_scale = 0.95,
	...
	inline std::vector<std::pair<int, int>> getCrosscheckedCorrespondences() { return corres_cross_checked_; }
   ...
   private:
	...
	template <typename T>
	void searchKDTree(const KDTree& tree, const T& input, std::array<int, 2>& indices, std::array<float, 2>& dists,
					  int nn);

	template <typename T>
	void searchKDTreeAll(KDTree* tree, const std::vector<T>& inputs, std::vector<int>& indices,
	...
	void runTupleTest(const std::vector<std::pair<int, int>>& corres, std::vector<std::pair<int, int>>& corres_out,
					  const float tuple_scale);

	// For a deeper understanding, please refer to Section III.D
	// ttps://arxiv.org/pdf/2409.15615
	void applyOutlierPruning(const std::vector<std::pair<int, int>>& corres,
							 std::vector<std::pair<int, int>>& corres_out, const std::string& robin_mode = "max_core");

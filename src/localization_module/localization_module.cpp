...
LocalizationModule::LocalizationModule(const std::string work_path){
    curr_dir_ = work_path;
    std::cout << "Current directory: " << curr_dir_ << std::endl;
...
void LocalizationModule::mapping_ctrl_cbk(const std_msgs::UInt32 &msg_in){
    ...
    }else if(msg.data == 3000){// 创建地图元素过程中，清除当前元素
        if(mapping_status_ == STARTPOINT_SET){
            clear_curr_element();
        }
        mapping_status_ = MAPPING_STARTED;
    }else if(msg.data == 4000){//设置终点
        if(mapping_status_ == STARTPOINT_SET){
    ...
    }else if(msg.data == 5000){// 重定位，并开始建图，/// TODO
        relocalize_and_mapping();
    }else if(msg.data == 9000){// 是否任何状态下均可退出建图 ？？？？
        // stop_mapping();
        // running_slam_ = false;
        // mapping_status_ = MODULE_INACTIVE;
    }else if(msg.data == 6000){// 开始定位，（先重定位，再定位）
...
void LocalizationModule::mark_end_point(int save_id){
    ...
    std::string pcd_path = std::string("~/work/AllLibs/Test/ws_map_manager/map/") + std::to_string(save_id)+std::string("/");
    if (save_id > 0){

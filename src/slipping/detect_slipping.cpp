...
bool DetectSlipping::detect_by_chassis_and_lidar(int& slip_flag) {
	...
	if (!lidar_queue_init_ || !chassis_queue_init_) {
		slipping_count_ = 0;
		// ROS_INFO_STREAM("detect waiting! lidar init = " << lidar_queue_init_  << ", chassis init = " <<
		// chassis_queue_init_);
		return false;
	}
...
void DetectSlipping::update_lidar_by_distance(geometry_msgs::PoseStamped pose) {
	...
	while (curr_time - pose_que_.front().header.stamp.toSec() > param_detect_window_time_range_ &&
		   pose_que_.size() >= 2) {
...
void DetectSlipping::update_lidar_by_segment(geometry_msgs::PoseStamped pose){
	...
	while(curr_time - pose_que_.front().header.stamp.toSec() > param_detect_window_time_range_ &&  pose_que_.size()
>=2){ double pop_dist = dist_que_.front(); lidar_sum_dist_ = lidar_sum_dist_ - pop_dist; pose_que_.pop();
		dist_que_.pop();
	}
	last_pose = curr_pose;
...
double DetectSlipping::cal_dist_along_heading(geometry_msgs::PoseStamped last_pose,

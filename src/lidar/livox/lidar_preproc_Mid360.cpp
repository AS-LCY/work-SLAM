...
LidarPreprocMid360::LidarPreprocMid360(rclcpp::Node::SharedPtr node) : LidarPreprocParent(node) { set_param(node); }

LidarPreprocMid360::~LidarPreprocMid360() {}

bool LidarPreprocMid360::pre_process(const sensor_msgs::msg::PointCloud2::SharedPtr ros_msg_in,
...
bool LidarPreprocMid360::msg2pcl_clip(const sensor_msgs::msg::PointCloud2::SharedPtr ros_msg_in,
									  PointCloudType::Ptr pcl_xyzin_out) {
	...
	for (std::uint32_t row = 0; row < ros_msg_in->height; ++row) {
		...
		for (std::uint32_t col = 0; col < ros_msg_in->width; ++col) {
			...
			if ((curpt->tag & 0x30) == 0x20 || (curpt->tag & 0x30) == 0x30 || (curpt->tag & 0x03) == 0x02 ||
				(curpt->tag & 0x03) == 0x03) {
				continue;
			}
...
// 间隔采样
void LidarPreprocMid360::extract_cloud_by_interval_sampling(const std::shared_ptr<livox_ros::LidarMsg> msg,
	...
	for (uint i = 1; i < plsize; i++) { // zd delete (msg->points[i].line < N_SCANS)
		if (((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00) &&
			...
			valid_num++;
			double range_square = msg->points[i].x * msg->points[i].x + msg->points[i].y * msg->points[i].y +
			...
			if (valid_num % point_filter_num_ == 0) {
				...
				curr_pt.curvature = msg->points[i].offset_time *
				...
				// // pl_full[i].curvature = msg->points[i].offset_time / float(1000000); // use curvature as time of
				// each laser points, curvature unit: ms pl_full[i].curvature = msg->points[i].offset_time *
				// float(1000); // use curvature as time of each laser points, curvature unit: ms if ((abs(pl_full[i].x
				// - pl_full[i - 1].x) > 1e-7) || (abs(pl_full[i].y - pl_full[i - 1].y) > 1e-7) || (abs(pl_full[i].z -
				// pl_full[i - 1].z) > 1e-7)) if ((abs(pl_full[i].x - pl_full[i - 1].x) > 0.15) || (abs(pl_full[i].y -
				// pl_full[i - 1].y) > 0.15) || (abs(pl_full[i].z - pl_full[i - 1].z) > 0.15))

				static PointType last_pt = temp_pt;
				if ((abs(curr_pt.x - last_pt.x) > 0.15) || (abs(curr_pt.y - last_pt.y) > 0.15) ||
					(abs(curr_pt.z - last_pt.z) > 0.15)) {
...
void LidarPreprocMid360::extract_cloud_by_interval_and_voxel(const std::shared_ptr<livox_ros::LidarMsg> msg,
															 PointCloudType::Ptr pcl_cld_out) {
	...
	for (uint i = 1; i < plsize; i++) { // zd delete (msg->points[i].line < N_SCANS)
		if (((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00) &&
			...
			if (valid_num % point_filter_num_ == 0) {
				...
				// || (abs(pl_full[i].z - pl_full[i - 1].z) > 1e-7)) if ((abs(pl_full[i].x - pl_full[i - 1].x) > 0.15)
				// || (abs(pl_full[i].y - pl_full[i - 1].y) > 0.15) || (abs(pl_full[i].z - pl_full[i - 1].z) > 0.15))
				// {
...
void LidarPreprocMid360::extract_cloud_by_simple_voxel(const std::shared_ptr<livox_ros::LidarMsg> msg,
													   PointCloudType::Ptr pcl_cld_out) {
	...
	for (size_t i = 0; i < plsize; i++) {
		...
		if (cur_pt->x > extent_xmin || cur_pt->x < extent_xmax || cur_pt->y > extent_ymin || cur_pt->y < extent_ymax ||
	...
	for (int i = 0; i < devision_cnt; i++) {
		...
		for (size_t j = 0; j < cur_cloud_sz; j++) {
			...
			if (!flag_if_fill[index]) {
				...
				// pl_full[cur_region_idx[j]].curvature = cur_pt->offset_time * float(1000); // use curvature as time of
				// each laser points, curvature unit: ms
...
// plane_judge()
// edge_jump_judge()
void LidarPreprocMid360::extract_cloud_by_feature(const std::shared_ptr<livox_ros::LidarMsg> msg,
												  PointCloudType::Ptr pcl_cld_out) {
	return;
}

...
bool LidarPreprocParent::set_common_params(rclcpp::Node::SharedPtr node){
    ...
    //     auto logger = rclcpp::get_logger("lidar_preproc");
    //     RCLCPP_ERROR(logger, "loaded_param is NULL");
    //     return false;
    // }else{
    ...
    //     std::cout << " ZZZ size() :" << loaded_param->lidar_preproc.z_range.size() << std::endl;
    //     for(int i =0; i < loaded_param->lidar_preproc.z_range.size();i++)
    //     {
    //         std::cout << " ZZZ :" << loaded_param->lidar_preproc.z_range[i] << std::endl;
    //     }

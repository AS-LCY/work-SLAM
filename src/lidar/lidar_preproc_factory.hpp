...
class LidarPreprocFactory {
	DECL_CLASSNAME(LidarPreprocFactory)
   public:
	static std::shared_ptr<LidarPreprocParent> new_lidar_preproc(const int lidar_type, rclcpp::Node::SharedPtr node) {
		...
		TRACE_INFO("valid lidar_type in factory: 1-lvx-Mid360 | 2-RS-Airy | 3-Vanjee722 | 4-HS-JT16 | 5-BS-M300");
		TRACE_INFO("curr lidar_type: %d", lidar_type);
		...
		if (lidar_type == 1) {
			...
			TRACE_ERR("Unknown lidar type(==%d) in lidar factory!", lidar_type);
			exit(0);
		}

...
class SCManager {
   ...
   public:
	...
	// double SC_DIST_THRES = 0.20; // empirically 0.1-0.2 is fine (rare false-alarms) for 20x60 polar context (but for
	// 0.15 <, DCS or ICP fit score check (e.g., in LeGO-LOAM) should be required for robustness)
	const double SC_DIST_THRES = 0.4; // 0.4-0.6 is good choice for using with robust kernel (e.g., Cauchy, DCS) + icp
									  // fitness threshold / if not, recommend 0.1-0.15

	// config
	const int TREE_MAKING_PERIOD_ =
		50; // i.e., remaking tree frequency, to avoid non-mandatory every remaking, to save time cost / if you want to

...
static Eigen::Matrix<double, 12, 12> process_noise_cov() {
...
//对应公式(2) 中的f
static Eigen::Matrix<double, 24, 1> get_f(state_ikfom s, input_ikfom in) {
	...
	Eigen::Vector3d a_inertial =
		s.rot.matrix() * (in.acc - s.ba); //  输入的imu的加速度，先转到世界坐标系（对应公式的第3行）
...
static Eigen::Matrix<double, 24, 24> df_dx(state_ikfom s, input_ikfom in) {
...
static Eigen::Matrix<double, 24, 12> df_dw(state_ikfom s, input_ikfom in) {

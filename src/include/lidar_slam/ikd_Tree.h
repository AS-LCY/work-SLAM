...
class KD_TREE {
   ...
   public:
	...
	using PointTypeCloud = typename pcl::PointCloud<PointType>::Ptr;
   ...
   public:
	...
	void set_cube_len(float cube_len) { cube_len = cube_len; }
	void set_det_range(float det_range) { det_range = det_range; }
	...
	void lasermap_fov_segment(Vector3d pos_lid); // zx add
	void map_incremental(PointTypeCloud feats_down_body, const vector<PointVector>& Nearest_Points,
						 bool flg_EKF_inited); // zx add
};

// template <typename T>
// T KD_TREE<T>::zeroP = T(0,0,0);

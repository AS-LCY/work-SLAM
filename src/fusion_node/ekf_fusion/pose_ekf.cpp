...
bool PoseEKF::input_check() {
    if (P_.rows() != P_.cols() || P_.rows() != status_num_ ||
       R_.rows() != R_.cols() || R_.rows() != measure_num_ ||
       M_.rows() != M_.cols() || M_.rows() != input_num_ ||
       dt_ <= 0) {
        return false;
    }
    return true;
...
void PoseEKF::set_dt(const double& dt) {
    dt_ = dt;
}

void PoseEKF::set_status(const Matrix& status){
    status_ = status*1.0;
}
...
Matrix PoseEKF::get_status_estimated() {
    return status_post_;
}

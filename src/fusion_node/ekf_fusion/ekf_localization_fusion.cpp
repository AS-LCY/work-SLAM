...
EkfLocalizationFusion::EkfLocalizationFusion() {
	...
	ekf_ptr_ = std::make_shared<PoseEKF>(status_num_, input_num_, measure_num_, status_covariance_, input_covariance_,
										 measure_covariance_, dt_, gating_params_);
	ROS_INFO("gating params: %f", gating_params_.dis_drift_lateral_thresh);
}

EkfLocalizationFusion::~EkfLocalizationFusion() { is_init_ = false; }

void EkfLocalizationFusion::localization_fusion_core(const flbot_msgs::LocalizationPoseData& pose_msg,
													 flbot_msgs::LocalizationPoseData* pose_msg_out) {
	pose_msg_ = pose_msg;
	*pose_msg_out = pose_msg;
	...
	geometry_msgs::Vector3 euler =
		localization_module::common::Quaternion::get_euler_zyx(pose_msg_out->fusion_pose.orientation);

	ROS_INFO("ekf -- dx: %8.3f, dy: %8.3f, dyaw: %7.4f", status_estimated_(0, 0) - measure_(0, 0),
			 status_estimated_(1, 0) - measure_(1, 0),
...
void EkfLocalizationFusion::set_localizationfusion_input() {
	...
	measure_(2, 0) =
		localization_module::common::Quaternion::get_euler_zyx(pose_msg_.slam_pose.orientation).z; // ekf 观测量: theta
	measure_(2, 0) =
		localization_module::common::NumericalProcess::unify_angle(measure_(2, 0)); // ekf 观测量: theta-unified
...
void EkfLocalizationFusion::init(const flbot_msgs::LocalizationPoseData& status) {
	pose_msg_ = status;
...
bool EkfLocalizationFusion::is_init() { return is_init_; }
...
bool EkfLocalizationFusion::matrix_init(const LocalizationFusionParams* lf_params) {
	if (lf_params->status_cov_mat_diag.size() != status_num_ || lf_params->input_cov_mat_diag.size() != input_num_ ||

...
inline tf::Transform odom_to_transform(const nav_msgs::Odometry odom_in) {
	tf::Transform res_transform;

	tf::Quaternion q;
	res_transform.setOrigin(
		tf::Vector3(odom_in.pose.pose.position.x, odom_in.pose.pose.position.y, odom_in.pose.pose.position.z));
...
inline nav_msgs::Odometry isometry3d_to_odom(const Eigen::Isometry3d isometry_in, std::string frame_in,
											 std::string child_frame_in) {

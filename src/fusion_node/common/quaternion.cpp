...
double Quaternion::rad2deg(double rad){
    return rad / M_PI * 180.0;
}


double Quaternion::deg2rad(double deg){
...
geometry_msgs::Vector3 Quaternion::rotate_vector3(const geometry_msgs::Vector3& vin,
    ...
    if (false == is_reverse_flag) {
        ans.x = r[0][0] * vin.x + r[0][1] * vin.y + r[0][2] * vin.z;    // To rotate a vector is to transform a point from one frame to another
        ans.y = r[1][0] * vin.x + r[1][1] * vin.y + r[1][2] * vin.z;
        ans.z = r[2][0] * vin.x + r[2][1] * vin.y + r[2][2] * vin.z;
    }
    else {
        ans.x = r[0][0] * vin.x + r[1][0] * vin.y + r[2][0] * vin.z;    // This is the reverse of the above since the matrix is transposed.

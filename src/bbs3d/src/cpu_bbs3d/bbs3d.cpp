...
void BBS3D::set_timeout_duration_in_msec(const int msec) { timeout_duration_ = std::chrono::milliseconds(msec); }
...
void BBS3D::calc_angular_info(std::vector<AngularInfo>& ang_info_vec) {
	...
	for (int i = max_level; i >= 0; i--) {
		...
		if (i == max_level) {
			...
			max_rpy_piece.x() =
				ang_info_vec[i + 1].rpy_res.x() != 0.0 ? ang_info_vec[i + 1].rpy_res.x() : max_rpy_.x() - min_rpy_.x();
			max_rpy_piece.y() =
				ang_info_vec[i + 1].rpy_res.y() != 0.0 ? ang_info_vec[i + 1].rpy_res.y() : max_rpy_.y() - min_rpy_.y();
			max_rpy_piece.z() =
				ang_info_vec[i + 1].rpy_res.z() != 0.0 ? ang_info_vec[i + 1].rpy_res.z() : max_rpy_.z() - min_rpy_.z();
		}
...
std::vector<DiscreteTransformation<double>> BBS3D::create_init_transset(const AngularInfo& init_ang_info) {
	const int init_transset_size =
		(init_tx_range_.second - init_tx_range_.first + 1) * (init_ty_range_.second - init_ty_range_.first + 1) *
				...
				for (int roll = 0; roll < init_ang_info.num_division.x(); roll++) {
					for (int pitch = 0; pitch < init_ang_info.num_division.y(); pitch++) {
						for (int yaw = 0; yaw < init_ang_info.num_division.z(); yaw++) {
							transset.emplace_back(
								DiscreteTransformation<double>(0, max_level, tx, ty, tz, roll, pitch, yaw));
						}
					}
				}
			}
...
void BBS3D::calc_score(DiscreteTransformation<double>& trans, const double trans_res, const Eigen::Vector3d& rpy_res,
					   const Eigen::Vector3d& min_rpy, const std::vector<Eigen::Vector4i>& buckets,
					   const int max_bucket_scan_count, const std::vector<Eigen::Vector3d>& points) {

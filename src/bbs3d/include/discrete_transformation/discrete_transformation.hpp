...
class DiscreteTransformation {
   public:
	...
	Eigen::Matrix<T, 4, 4> create_matrix(const T trans_res, const Eigen::Matrix<T, 3, 1>& rpy_res,
										 const Eigen::Matrix<T, 3, 1>& min_rpy) {
		Eigen::Translation<T, 3> translation(x * trans_res, y * trans_res, z * trans_res);
		Eigen::AngleAxis<T> rollAngle(roll * rpy_res.x() + min_rpy.x(), Eigen::Matrix<T, 3, 1>::UnitX());
		Eigen::AngleAxis<T> pitchAngle(pitch * rpy_res.y() + min_rpy.y(), Eigen::Matrix<T, 3, 1>::UnitY());
		Eigen::AngleAxis<T> yawAngle(yaw * rpy_res.z() + min_rpy.z(), Eigen::Matrix<T, 3, 1>::UnitZ());
		return (translation * yawAngle * pitchAngle * rollAngle).matrix();
	}

	void branch(std::vector<DiscreteTransformation>& b, const int child_level, const int v_rate,
					...
					for (int l = 0; l < num_division.x(); l++) {
						for (int m = 0; m < num_division.y(); m++) {
							for (int n = 0; n < num_division.z(); n++) {
								b.emplace_back(DiscreteTransformation(0, child_level, x * v_rate + i, y * v_rate + j,
																	  z * v_rate + k, roll * num_division.x() + l,
	...
	std::vector<DiscreteTransformation> branch(const int child_level, const int v_rate,
											   const Eigen::Vector3i& num_division) {

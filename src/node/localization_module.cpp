...
LocalizationModule::LocalizationModule( rclcpp::Node::SharedPtr node,  ModuleStatus init_status):node_(node), br_(node_),tf_buffer_(node_->get_clock()), tf_listener_(tf_buffer_)
// LocalizationModule::LocalizationModule(/*const std::string work_path,*/ ModuleStatus init_status)
{
    ...
    if(!init_module_by_set_status(init_status)){
         RCLCPP_INFO(node_->get_logger(),"Try to init module with status: %s, but failed",print_ModuleStatus(init_status).c_str());
    }else{
         RCLCPP_INFO(node_->get_logger(),"Localization Module Start with status:\033[1;32m %s\033[0m", print_ModuleStatus(running_module_status_.load()).c_str());
    }
     RCLCPP_INFO(node_->get_logger(),"***************************************************");
...
bool LocalizationModule::create_ROS_IO(){
    ...
    sub_pointcloud2_ = node_->create_subscription<sensor_msgs::msg::PointCloud2>( slam_param_.lidar_preproc.sub_lidar_topic,
        10,
        std::bind(&LocalizationModule::lidar_ros_callback, this, std::placeholders::_1)
    );

    std::cout << "=========================  slam_param_.lidar_preproc.sub_lidar_topic:  " <<  slam_param_.lidar_preproc.sub_lidar_topic<< std::endl;
    ...
    pub_localization_module_health_ = node_->create_publisher<fairland_msgs::msg::LocalizationModuleHealth>(
        slam_param_.common.pub_topic_module_health,
        rclcpp::QoS(100)
    );
    ...
    pub_log_ = node_->create_publisher<std_msgs::msg::Float64MultiArray>(
        slam_param_.common.pub_topic_module_loginfo,
        rclcpp::QoS(100)
    );
    ...
    pubOdomAftMapped = node_->create_publisher<nav_msgs::msg::Odometry>(
        "/Odometry_lidar_in_map",
        rclcpp::QoS(100)
    );
...
this->timer_slam_ = node_->create_wall_timer(
    ...
    this->sub_mapping_ctrl_ = node_->create_subscription<std_msgs::msg::UInt32>(
        ...
        std::bind(&LocalizationModule::localization_module_ctrl_callback, this, std::placeholders::_1),
        sub_options
    );
    ...
    this->timer_module_status_  = node_->create_wall_timer(
        std::chrono::milliseconds(100), // 100ms = 10Hz
        std::bind(&LocalizationModule::pub_module_status_timer, this),
        ctrl_callback_group_
    );
    ...
    pub_body_cloud_filter_ = node_->create_publisher<sensor_msgs::msg::PointCloud2>("/flbot/localization/body_cloud_filter", 20);
    ...
    pubLoopConstraintEdge = node_->create_publisher<visualization_msgs::msg::MarkerArray>("/flbot/mapping/loop_closure_constraints", 1);
...
void LocalizationModule::slam_dealt_timer(){   // TODO : 主函数{   // TODO : 主函数
    ...
    if(slam_timer_interval < 0){
      //  ROS_ERROR_STREAM(RED << "slam main thread time jump back, this_time - last_time = " << slam_timer_interval << " seconds" << RESET);
        RCLCPP_ERROR(
            node_->get_logger(),
            "slam main thread time jump back, this_time - last_time = %.3f seconds",
            slam_timer_interval
        );
...
//       ROS_ERROR_STREAM(RED << "slam main thread time jump to future, this_time - last_time = " << slam_timer_interval << " seconds" << RESET);
            RCLCPP_ERROR(
                node_->get_logger(),
                "slam main thread time jump to future, this_time - last_time =  %.3f seconds",
                slam_timer_interval
            );
    }
    ...
    if (curr_running_module_status == ModuleStatus::MODULE_IDLE){
        ...
        RCLCPP_INFO_STREAM_EXPRESSION(node_->get_logger(),condition, "slam dealt: running module status: " << print_ModuleStatus(curr_running_module_status));
    ...
    // TODO : 发布一下 建图点云信息
    if(is_mapping_status(curr_running_module_status) && mapping_status_.load() == 3){ // mapping_status_ 是建图状态  slam 内部修改
    ...
    }else if(curr_running_module_status == ModuleStatus::MODULE_LOCALIZATION && localization_status_is_ok(localization_status_now)){// TODO: 忽略
...
int LocalizationModule::check_fill_health_msg(ModuleStatus curr_running_module_status, fairland_msgs::msg::LocalizationModuleHealth &health_msg){
...
void LocalizationModule::check_fill_module_status_msg(ModuleStatus curr_running_module_status, fairland_msgs::msg::LocalizationModuleStatus &status_msg){
    ...
    if(curr_running_module_status == ModuleStatus::MODULE_IDLE){
        ...
        RCLCPP_ERROR(node_->get_logger(),"error running module status: %s", print_ModuleStatus(curr_running_module_status).c_str());
    }
    ...
    if(status_msg.localization_status != 0 && status_msg.localization_status != 3){
        ...
        RCLCPP_WARN(node_->get_logger(),"[Status Timer]: localization_status: %d" , int(status_msg.localization_status) );
    }
...
*/
    ...
    if(curr_running_module_status == ModuleStatus::MODULE_LOCALIZATION && localization_status_is_ok(localization_status_.load())){
        publish_odometry_lidar_in_map(slam_->getLidarInMap() * T_lidar_baselink_, slam_->get_current_pose(), "map", "base_link", curr_running_module_status);
...
void LocalizationModule::fill_module_l_status(ModuleStatus curr_running_module_status, fairland_msgs::msg::LocalizationModuleStatus &status_msg){
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////
    if(node_status == 0){
        status_msg.localization_status = int(LocalizationStatus::L_INACTIVE); //fairland_msgs::LocalizationModuleStatus::L_INACTIVE;
        ...
        if( slam_run_status == 1){ // slam_run_status = normal
            ...
            status_msg.localization_status = int(LocalizationStatus::L_FAILED); //fairland_msgs::LocalizationModuleStatus::L_FAILED;
...
void LocalizationModule::fill_module_m_status(ModuleStatus curr_running_module_status, fairland_msgs::msg::LocalizationModuleStatus &status_msg){
    if(curr_running_module_status != ModuleStatus::MODULE_MAPPING && 
        curr_running_module_status != ModuleStatus::MODULE_SEC_MAPPING){
...
bool LocalizationModule::init_module_by_set_status(ModuleStatus set_status){
    ...
    if(set_status == ModuleStatus::MODULE_IDLE){
     ...
     RCLCPP_INFO(node_->get_logger(),"init module status: %s", print_ModuleStatus(running_module_status_.load()).c_str());

...
bool LocalizationModule::start_mapping(int map_id){
    ...
    auto running_module_status_now = running_module_status_.load();
     RCLCPP_INFO(node_->get_logger(),"Module Status for now: %s", print_ModuleStatus(running_module_status_now).c_str());
    ...
    if (running_module_status_now == ModuleStatus::MODULE_IDLE){
        ...
        return true;
         ...
         RCLCPP_INFO(node_->get_logger(),"skip, already running mapping now");//////////////////// TODO 要不要重置 start_index_ end_index_ ？？？
        return false;
        ...
        return false;
         ...
         RCLCPP_INFO(node_->get_logger(),"running_module_status_now: %s", print_ModuleStatus(running_module_status_now).c_str());
...
bool LocalizationModule::stop_mapping_without_saving_map(){
    ...
    //          last_running_module_status_ == ModuleStatus::MODULE_LOCALIZATION){
         ...
         RCLCPP_INFO(node_->get_logger(),"skip, can not stop mapping, running_module_status_: %s", print_ModuleStatus(running_module_status_now).c_str());
        return false;
    }else{
         RCLCPP_INFO(node_->get_logger(),"skip, status error!");
         RCLCPP_INFO(node_->get_logger(),"running_module_status_now: %s", print_ModuleStatus(running_module_status_now).c_str());
...
bool LocalizationModule::stop_mapping(){
    ...
    if (is_mapping_status(running_module_status_now)){
        if(mapping_status_now == 3){
         ...
         RCLCPP_INFO(node_->get_logger(),"skip, can not stop mapping, running_module_status_: %s", print_ModuleStatus(running_module_status_.load()).c_str());
        return false;
    }else{
         RCLCPP_INFO(node_->get_logger(),"skip, status error!");
         RCLCPP_INFO(node_->get_logger(),"running_module_status_now: %s", print_ModuleStatus(running_module_status_now).c_str());
...
bool LocalizationModule::stop_localization(){
    ...
    if (running_module_status_now == ModuleStatus::MODULE_LOCALIZATION){
        ...
        return true;
         ...
         RCLCPP_INFO(node_->get_logger(),"skip, can not stop localization, running_module_status_: %s", print_ModuleStatus(running_module_status_now).c_str());
        return false;
    }else{
         RCLCPP_INFO(node_->get_logger(),"skip, status error!");
         RCLCPP_INFO(node_->get_logger(),"last_running_module_status_: %s", print_ModuleStatus(running_module_status_now).c_str());
         RCLCPP_INFO(node_->get_logger(),"set_module_status: %s", print_ModuleStatus(set_status).c_str());
         RCLCPP_INFO(node_->get_logger(),"running_module_status_: %s", print_ModuleStatus(running_module_status_now).c_str());
         RCLCPP_INFO(node_->get_logger(),"mapping_status: %d", mapping_status_.load());
         RCLCPP_INFO(node_->get_logger(),"localization_status: %d", localization_status_.load());
         RCLCPP_INFO(node_->get_logger(),"****************************");
        return false;
...
bool LocalizationModule::restart_second_mapping( int map_id){
    ...
    if(curr_running_module_status == ModuleStatus::MODULE_SEC_MAPPING){    
        if(!stop_mapping_without_saving_map()){
         ...
         RCLCPP_INFO(node_->get_logger(),"skip, can not restart sec_mapping, running_module_status_: %s", print_ModuleStatus(curr_running_module_status).c_str());
        return false;
    }else{
         RCLCPP_INFO(node_->get_logger(),"skip, status error!");
         RCLCPP_INFO(node_->get_logger(),"running_module_status_: %s", print_ModuleStatus(curr_running_module_status).c_str());
         RCLCPP_INFO(node_->get_logger(),"mapping_status: %d", mapping_status_.load());
         RCLCPP_INFO(node_->get_logger(),"localization_status: %d", localization_status_.load());
         RCLCPP_INFO(node_->get_logger(),"****************************");
        return false;
...
bool LocalizationModule::make_slam_obj(lidar_slam::LidarSlamParam yaml_param, ModuleStatus set_status){
    ...
    if(set_status == ModuleStatus::MODULE_MAPPING){
        ...
        std::cout << RED << "ModuleStatus: " << print_ModuleStatus(set_status).c_str() << ", status error!" <<RESET << std::endl;
        return false;
    }
     RCLCPP_INFO(node_->get_logger(),"Making obj(lidar_slam) --- with: set_slam_mode = %s", lidar_slam::print_SlamWorkMode(set_slam_mode).c_str());
    slam_ = std::make_unique<lidar_slam::LidarSlam>(yaml_param, set_slam_mode, node_); //////////////////////////////////////////////TODO::
...
bool LocalizationModule::make_map_directory_name(int map_id){
    ...
    if(slam_param_.common.run_on_mower){
        ...
        //    std::cout << "Load param: /map_manager/map_folder failed, using default param, map_folder: " << slam_param_.common.map_directory << std::endl;

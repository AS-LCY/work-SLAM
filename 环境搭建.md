...
# flbot_msgs
git clone git@e.coding.net:g-snex1877/flbot_project/flbot_msgs.git
# flbot_thirdparty
...
#（运行 devel 中的可执行文件需要， 运行 install 中的可执行文件则不需要）
...
# 可以每个包分开编译
catkin_make --pkg flbot_msgs
catkin_make --pkg livox_ros_driver2
catkin_make --pkg lidar_slam
catkin_make install
```
...
- 路径： ./localization_module/config/lidar_slam_param.yaml
...
- 启动节点的初始化模式：
  ...
  # line 2 in yaml
  common:
    init_module_status: 3 #  默认是 0：IDLE
    
  # 如果启动初始化是定位模式，需要修改 定位的加载地图路径
     
  # line 54 in yaml
  localization:
    load_map_dir: "/home/panmiaomiao/catkin_ws/data/23楼电梯外" # 修改地图地址
  ```
...
- 解决办法
  ...
  - 升级步骤：（测试可用）
    ...
    cmake -DGTSAM_BUILD_METIS_EXECUTABLES=ON -DGTSAM_USE_SYSTEM_EIGEN=ON ..

    sudo sh -c 'echo "/usr/local/lib" >> /etc/ld.so.conf.d/gtsam.conf'
